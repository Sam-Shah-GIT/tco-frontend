...
class tco_model:
    ...
    def __init__(self, coating_type="XGIT-Fuel", cleaning_frequency=None, average_power=4000, fuel_type="VLSFO",
                 vessel_efficiency=0.7, fixed_cleanings=None, debug=True, region="equatorial",
                 fouling_type="calcareous", growth_type="gaussian", reactive_cleaning=None, activity_rate=0.5,
                 max_speed=20,imo_from_clarksons = None,cleaning_subscription = None,coated_sqm = 800,charter_rate = 25000,offhire_cost_per_cleaning = 10000,m_modifier = 1):
    ...
    def load_fuel_details(self, fuel_type):
        ...
        file_path = r"C:\Dashboard\tco_ui\tco_ui\tco\fuel_prices_today.csv"

    
        # load the table of fuel details
        self.fuel_details = pd.read_csv(file_path)
    ...
    def load_coating_details(self, coating_type):
        ...
        project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        file_path = r'C:\Dashboard\tco_ui\tco_ui\tco\coating_details 1.xlsx'
        

        # Load the table of coating details using the constructed path
        self.coating_details = pd.read_excel(file_path, skiprows=5)
    ...
    def run_5y(self):
        ...
        for day in range(1, days_in_5_years):
            ...
            if self.debug:
                ...
                print(f"Daily power loss is {self.daily_power_loss} - {self.coating_type}")
        ...
        for daily_power_output in power_output:
        ...
        fuel_consumptions_daily = [x for x in fuel_consumptions_daily if x != 0]
        fuel_consumptions_daily = np.mean(fuel_consumptions_daily)
        self.operating_daily_fuel_consumption = round(fuel_consumptions_daily, 2)
...
def plot_results_obj_list(models, col='power_output', no_power_details=True, growth_type="linear", save=False,
    ...
    # Create a figure with custom size. Adjust the subplot size dynamically based on the number of models
    fig, axs = plt.subplots(1, 2, figsize=(40, 16), gridspec_kw={'width_ratios': [3, 1]})
...
def plot_coating_details(models, metrics=['power_change', 'power_output', 'total_energy', 'coating_thickness',
                                          'coating_roughness'], save=False, debug=False):
    """
...
def create_report(models, output_path='output.pdf', debug=False):
    ...
    model_results_fig = plot_results_obj_list(models_results, col='power_change', no_power_details=False, save=True, debug=debug,show = False,show_table = False)
    cii_table_fig = cii_table(models_cii, save=True, debug=debug,show = False)
    coating_details_fig = plot_coating_details(models_coating_details, save=True, debug=debug,show = False)
